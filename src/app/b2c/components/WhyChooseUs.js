...
<CenterWrapper className={"space-y-8"}>
  <MainHeading className={"text-black font-bold"}>Why Buyers choose us?</MainHeading>
  <div className='flex items-center bg-[#0D68FB] rounded-full w-fit '>
{[["Buyers", true], ["Suppliers", false]].map(([item, active],idx)=>(
  <div key={idx} className={`py-2 ${active?"bg- text-white" :" bg-white text-black" }  rounded-full w-24 text-center`}>
{item}
  </div>
))}
  </div>
...
{arr.map(({icon, heading, description},idx)=>(
  <Card key={idx} icon={icon} heading={heading} description={description} idx={idx} />
))}
  </div>
</CenterWrapper>
    </div>
...
const Card= ({icon, heading, description, idx})=> {
  return (
    <div className='bg-[#FFFEFE4D] rounded-xl p-4 space-y-10 shadow-lg'>
      <div className={`text-4xl w-fit p-8 ${idx==0 ?"bg-[#ECDAF2]" : idx==1 ?"bg-[#FFEDDE]" : idx==2?"bg-[#D9FCF6]" : "bg-[#E6F7FF]"} bg-white rounded-full mx-auto`}>
        {icon}
      </div>
