...
const ProductLaunches = () => {
  ...
  return (
    <CenterWrapper className="">
      <MainHeading className=" font-bold mb-8 text-center">New Launches</MainHeading>
      <Slider {...settings}>
        {products.map((product, index) => (
          <div key={product.id} className="px-8">
            <a href={product.link} className="block">
              <div className={`rounded-t-full pt-20 ${bgColors[index % 4]} relative aspect-[3/4] flex items-center `}>
                {/* Sparkle icons */}
